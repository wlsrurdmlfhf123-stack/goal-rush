...
export const B = {
   ...
   *
   * [0.45 -> 0.6] 0.45면 90도로 홱 꺾을 때 공이 옛 방향 운동량의 55%를 그대로
   * 안고 흘러 나가 발 범위(range 2.6) 밖까지 달아났다(계측: 직진 1.58m ->
   * 회전 후 3.75m = 놓침, 쫓아가야 함). 0.6이면 방향 전환 터치 한 방으로
   * 공이 2.3~2.4m에 남아 드리블이 계속 붙는다. 여전히 40%는 흘러 나가므로
   * "천천히 도는 회전"이나 지그재그에서는 공을 놓친다 = 급회전의 위험은 남는다.
   * 직진 터치(align >= turnAlign 경로)와 킥·개인기는 이 값을 안 쓴다.
   */
  turnBite: 0.6,
  /**
