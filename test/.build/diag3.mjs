...
function build() {
  ...
  const rag = createRagdoll(
    physics,
    new THREE2.Scene(),
    new CANNON3.Vec3(0, P.rideHeight + 0.15, 0),
    playerMat,
...
function mkFridge(physics, propMat, z) {
  const b = new CANNON3.Body({
    mass: 20,
    shape: new CANNON3.Box(new CANNON3.Vec3(0.6, 1.1, 0.5)),
    position: new CANNON3.Vec3(0, 1.1, z),
    material: propMat
  });
...
var FWD = { moveX: 0, moveZ: 1, jump: false };
function runCarry(label, gripMode) {
  ...
  for (let i = 0; i < 480; i++) {
    ...
    if (gripMode) {
      ...
      if (liftable && !constraintL) {
        const c = new CANNON3.PointToPointConstraint(rag.handL, new CANNON3.Vec3(0, 0, 0), fridge, new CANNON3.Vec3(0, 0, -0.5), holdForce);
        physics.addConstraint(c);
        constraintL = c;
      } else if (!liftable && constraintL) {
        physics.removeConstraint(constraintL);
        constraintL = null;
      }
    }
    rag.setHeld([fridge]);
    ...
    if (i === 120 || i === 240 || i === 479) {
      console.log(`    t=${(i / 60).toFixed(1)}s fridgeZ=${fridge.position.z.toFixed(3)} fridgeY=${fridge.position.y.toFixed(3)} pelvisZ=${rag.pelvis.position.z.toFixed(3)} pelvisY=${rag.pelvis.position.y.toFixed(2)} state=${rag.state}`);
    }
  }
...
console.log("=== 2\uC778 grab (\uB4E4\uC5B4\uC62C\uB9AC\uAE30) ===");
{
  ...
  const playerMat = new CANNON3.Material("player2");
  physics.addContactMaterial(new CANNON3.ContactMaterial(propMat, playerMat, { friction: 0.3, restitution: 0.05 }));
  const rags = [];
  for (const [i, x] of [[1, -0.45], [2, 0.45]]) {
    const g = groupFor(i);
    rags.push(createRagdoll(
      physics,
      new THREE2.Scene(),
      new CANNON3.Vec3(x, P.rideHeight + 0.15, 0),
      playerMat,
      { skin: 16764057, shirt: 4164863, pants: 3355460 },
      g,
      ragdollMask(g)
  ...
  const fridge = mkFridge(physics, propMat, 0.9);
  for (let i = 0; i < 240; i++) {
    for (const r of rags) r.control(1 / 60, NONE, physics);
    physics.step(1 / 60);
