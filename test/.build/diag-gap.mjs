...
function createBallPlay() {
  ...
  function dribble(rag, ball, dt, carrying) {
    ...
    if (stateOf(rag).lockout > 0) return;
    if (!moving(rag)) return;
    ...
    let ax = (tx - ball.position.x) * B.kp - (ball.velocity.x - pv.x) * B.kd;
    let az = (tz - ball.position.z) * B.kp - (ball.velocity.z - pv.z) * B.kd;
    ...
    if (dist < B.pokeRange && st.pokeTimer <= 0) {
      ...
      ball.applyImpulse(new CANNON2.Vec3(dir.x * j, 0, dir.z * j));
  ...
  function tryTrick(rag, ball, carrying) {
    ...
    const side = (ball.position.x - p.x) * sx + (ball.position.z - p.z) * sz;
    ...
    rag.torso.torque.y += -sgn * B.trickTorque;
...
function run(tune) {
  ...
  const gm = new CANNON3.Material("ground"), bm = new CANNON3.Material("player"), bam = new CANNON3.Material("ball");
  physics.addContactMaterial(new CANNON3.ContactMaterial(gm, bm, { friction: 0.55, restitution: 0 }));
  physics.addContactMaterial(new CANNON3.ContactMaterial(gm, bam, { friction: 0.32, restitution: 0.45 }));
  physics.addContactMaterial(new CANNON3.ContactMaterial(bm, bam, { friction: 0.28, restitution: 0.35 }));
  const g = new CANNON3.Body({ type: CANNON3.Body.STATIC, shape: new CANNON3.Plane(), material: gm, collisionFilterGroup: GROUP_WORLD, collisionFilterMask: -1 });
  ...
  const ball = new CANNON3.Body({ mass: B.mass, shape: new CANNON3.Sphere(B.radius), position: new CANNON3.Vec3(0, B.radius + 0.01, -1), material: bam });
  ...
  for (let i = 0; i < 260; i++) {
    step(0, -1);
    if (i > 90) gaps.push(Math.hypot(ball.position.x - rag.pelvis.position.x, ball.position.z - rag.pelvis.position.z));
  }
  const avg = gaps.reduce((a, b) => a + b, 0) / gaps.length;
  return {
    \uD3C9\uADE0\uAC04\uACA9: +avg.toFixed(2),
    \uCD5C\uC18C: +Math.min(...gaps).toFixed(2),
    \uCD5C\uB300: +Math.max(...gaps).toFixed(2),
    \uC0C1\uD0DC: rag.state,
    \uACF5\uC774\uB3D9: +Math.abs(ball.position.z).toFixed(1)
  };
}
var base = { ahead: B.ahead, pokeRange: B.pokeRange, pokeImpulse: B.pokeImpulse, pokeBySpeed: B.pokeBySpeed, pokeInterval: B.pokeInterval, accel: B.accel };
var cands = [
  { name: "\uD604\uC7AC", t: {} },
  { name: "ahead 1.15", t: { ahead: 1.15 } },
  { name: "ahead 1.15 + poke 0.9/1.8/0.30", t: { ahead: 1.15, pokeRange: 0.9, pokeImpulse: 1.8, pokeBySpeed: 0.3 } },
  { name: "ahead 1.3 + poke 1.0/2.2/0.35 + int 0.22", t: { ahead: 1.3, pokeRange: 1, pokeImpulse: 2.2, pokeBySpeed: 0.35, pokeInterval: 0.22 } },
  { name: "ahead 1.3 + poke 1.0/2.6/0.40 + accel 14", t: { ahead: 1.3, pokeRange: 1, pokeImpulse: 2.6, pokeBySpeed: 0.4, pokeInterval: 0.22, accel: 14 } }
];
for (const c of cands) {
  Object.assign(B, base);
  console.log(c.name.padEnd(42), JSON.stringify(run(c.t)));
}
