...
function createRagdoll(physics2, scene2, origin, material, colors, group, otherMask) {
...
// test/.diag.ts
var physics = new CANNON2.World({ gravity: new CANNON2.Vec3(0, -18, 0) });
physics.broadphase = new CANNON2.SAPBroadphase(physics);
physics.allowSleep = false;
physics.solver.iterations = 22;
physics.solver.tolerance = 5e-4;
var groundMat = new CANNON2.Material("ground");
var playerMat = new CANNON2.Material("player");
var propMat = new CANNON2.Material("prop");
...
var ground = new CANNON2.Body({ type: CANNON2.Body.STATIC, shape: new CANNON2.Plane(), material: groundMat });
ground.quaternion.setFromEuler(-Math.PI / 2, 0, 0);
physics.addBody(ground);
var fridge = new CANNON2.Body({
  mass: 20,
  shape: new CANNON2.Box(new CANNON2.Vec3(0.6, 1.1, 0.5)),
  position: new CANNON2.Vec3(0, 1.1, 2.2),
  material: propMat
});
...
for (let i = 0; i < 400; i++) {
  ...
  if (i % 10 === 0 || nc > 0 && i % 5 === 0) {
    console.log(
      `${String(i).padStart(4)}  ${rag.pelvis.position.z.toFixed(3).padStart(7)}  ${rag.pelvis.position.y.toFixed(3).padStart(7)}  ${fridge.position.z.toFixed(3).padStart(7)}  ${fridge.position.y.toFixed(3).padStart(7)}  ${String(nc).padStart(8)}  ${rag.torso.position.z.toFixed(3).padStart(6)}  ${rag.parts.get("head").body.position.z.toFixed(3).padStart(6)}  ${rag.parts.get("footL").body.position.z.toFixed(3).padStart(6)}`
    );
  }
}
