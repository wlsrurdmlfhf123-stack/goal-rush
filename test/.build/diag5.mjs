...
function build(cfg) {
  ...
  physics.addContactMaterial(new CANNON2.ContactMaterial(playerMat, propMat, { friction: cfg.fric ?? 0.3, restitution: 0.05 }));
...
function penetrationTest(label, cfg) {
  const { physics, rag, propMat } = build(cfg);
  const fridge = new CANNON2.Body({ mass: 20, position: new CANNON2.Vec3(0, 1.1, 2.2), material: propMat });
  fridge.addShape(new CANNON2.Box(new CANNON2.Vec3(0.6, 1.1, 0.5)), new CANNON2.Vec3(0, cfg.comDrop ?? 0, 0));
  ...
  console.log(
    `  ${label.padEnd(30)} \uD1B5\uACFC=${passedThrough ? "\uC608 \u2717" : "\uC544\uB2C8\uC624 \u2713"}  \uCD5C\uC18C\uAC04\uACA9=${minGap.toFixed(2).padStart(7)}  \uCD5C\uACE0\uACE8\uBC18=${maxPelvisY.toFixed(2)}  \uB0C9\uC7A5\uACE0z=${fridge.position.z.toFixed(2).padStart(6)}  \uC138\uC6CC\uC9D0=${up.y > 0.8 ? "\uC608 \u2713" : "\uC544\uB2C8\uC624 \u2717"}(up=${up.y.toFixed(2)})`
  );
}
console.log("=== \uD50C\uB808\uC774\uC5B4<->\uC18C\uD488 \uB9C8\uCC30 / \uBB34\uAC8C\uC911\uC2EC\uC5D0 \uB530\uB978 \uB4F1\uBC18\xB7\uC804\uBCF5 (10\uCD08 \uB3CC\uC9C4) ===");
penetrationTest("\uD604\uC7AC (friction 0.3)", {});
penetrationTest("friction 0.15", { fric: 0.15 });
penetrationTest("friction 0.05", { fric: 0.05 });
penetrationTest("friction 0.0", { fric: 0 });
...
console.log("\n=== \uC548\uC815\uC131 \uD68C\uADC0 (\uAC00\uB9CC\uD788 \uC11C \uC788\uAE30 10\uCD08) ===");
for (const [label, cfg] of [["\uD604\uC7AC", {}], ["friction 0.05", { fric: 0.05 }]]) {
