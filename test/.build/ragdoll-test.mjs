...
function build() {
  ...
  physics.solver.iterations = 20;
  const groundMat = new CANNON2.Material("ground");
  const bodyMat = new CANNON2.Material("body");
  physics.addContactMaterial(new CANNON2.ContactMaterial(groundMat, bodyMat, { friction: 0.55, restitution: 0 }));
  ...
  const rag = createRagdoll(
    physics,
    scene,
    new CANNON2.Vec3(0, P.rideHeight, 0),
    bodyMat,
...
function step(physics, rag, n, input = NONE) {
  for (let i = 0; i < n; i++) {
...
console.log("\n--- TEST 1: \uC778\uAC04\uD615 \uAD6C\uC131 \uD655\uC778 ---");
{
  ...
  check(
    "\uBAA8\uB4E0 \uD30C\uCE20 \uC774\uB984 \uC874\uC7AC",
    expected.every((n) => rag.parts.has(n)),
...
console.log("\n--- TEST 2: \uAC00\uB9CC\uD788 \uB450\uBA74 \uC2A4\uC2A4\uB85C \uC11C \uC788\uB294\uAC00 (Active Ragdoll) ---");
{
  ...
  check(
    "\uBA38\uB9AC\uAC00 \uACE8\uBC18\uBCF4\uB2E4 \uC704",
    head.position.y > rag.pelvis.position.y + 0.5,
...
console.log("\n--- TEST 5: \uB118\uC5B4\uC9C4 \uD6C4 \uC2A4\uC2A4\uB85C \uC77C\uC5B4\uB098\uB294\uAC00 (RECOVERY) ---");
{
  ...
  check(
    "\uC77C\uC5B4\uB09C \uB4A4 \uB2E4\uC2DC \uC11C \uC788\uC74C (\uCD5C\uADFC 1\uCD08 \uD3C9\uADE0 \uACE8\uBC18\uB192\uC774 >0.55)",
    avgHeight > 0.55,
...
console.log("\n--- TEST 7: \uADF9\uB2E8 \uCDA9\uACA9 \uC5F0\uD0C0\uC5D0\uB3C4 \uBB3C\uB9AC \uD3ED\uBC1C\uC774 \uC5C6\uB294\uAC00 ---");
{
  ...
  for (let i = 0; i < 1200; i++) {
    const s = i % 2 === 0 ? 1 : -1;
    rag.torso.applyImpulse(new CANNON2.Vec3(70 * s, 25, -55 * s), rag.torso.position);
    rag.parts.get("head").body.applyImpulse(new CANNON2.Vec3(-40 * s, 10 * s, 35 * s), rag.parts.get("head").body.position);
...
console.log("\n--- TEST 8: \uC790\uAE30 \uD30C\uCE20\uB07C\uB9AC \uCDA9\uB3CC\uD558\uC9C0 \uC54A\uB294\uAC00 ---");
{
  ...
  for (let i = 0; i < 400; i++) {
    rag.torso.applyImpulse(new CANNON2.Vec3(18, 0, 18), rag.torso.position);
...
console.log("\n--- TEST 9: \uB450 \uB798\uADF8\uB3CC\uC774 \uC11C\uB85C \uCDA9\uB3CC\uD558\uB294\uAC00 (\uD50C\uB808\uC774\uC5B4 \uAC04 \uCDA9\uB3CC) ---");
{
  ...
  physics.addBody(ground);
  const scene = new THREE2.Scene();
  const a = createRagdoll(
    physics,
    scene,
    new CANNON2.Vec3(-0.75, P.rideHeight, 0),
    bodyMat,
    { skin: 16764057, shirt: 4164863, pants: 3355460 },
    GROUP_A,
    GROUP_WORLD | GROUP_B
  );
  const b = createRagdoll(
    physics,
    scene,
    new CANNON2.Vec3(0.75, P.rideHeight, 0),
    bodyMat,
    { skin: 16764057, shirt: 16737860, pants: 3355460 },
    GROUP_B,
    GROUP_WORLD | GROUP_A
  );
...
console.log("\n--- \uC218\uCE58 \uC548\uC815\uC131: \uAC10\uC1E0\uD56D\uC774 \uBC1C\uC0B0\uD558\uC9C0 \uC54A\uB294\uAC00 ---");
{
  ...
  check(
    "\uAC00\uB9CC\uD788 \uC11C \uC788\uC744 \uB54C \uAD00\uC808\uC774 \uBC8C\uC5B4\uC9C0\uC9C0 \uC54A\uB294\uB2E4 (< 2cm)",
    maxGap < 0.02,
    `maxGap=${maxGap.toFixed(3)}m`
  );
  check(
    "\uAC01\uC18D\uB3C4\uAC00 \uC548\uC804 \uD074\uB7A8\uD504(20 rad/s)\uC5D0 \uB2FF\uC9C0 \uC54A\uB294\uB2E4",
    capHits === 0 && maxW < 10,
    `maxAngVel=${maxW.toFixed(1)} capHits=${capHits}`
  );
  check(
    "\uC81C\uC790\uB9AC\uC5D0\uC11C \uB4E4\uC369\uC774\uC9C0 \uC54A\uB294\uB2E4 (\uACE8\uBC18 \uC9C4\uD3ED < 2cm)",
    maxY - minY < 0.02,
    `amp=${(maxY - minY).toFixed(3)}m`
  );
  ...
  check(
    "\uAC10\uC1E0 \uACC4\uC218\uB294 \uC5EC\uC804\uD788 \uBC1C\uC0B0 \uC601\uC5ED\uC774\uB2E4 = \uC0C1\uD55C\uC774 \uC2E4\uC81C\uB85C \uC77C\uD558\uACE0 \uC788\uB2E4",
    worst.some((r) => r > 2),
    `r=${worst.map((r) => r.toFixed(2)).join(", ")}`
  );
}
console.log("\n--- \uC218\uCE58 \uC548\uC815\uC131: \uAC77\uAE30/\uAE09\uD68C\uC804/\uC810\uD504\uC5D0\uC11C\uB3C4 \uC720\uC9C0\uB418\uB294\uAC00 ---");
{
  ...
  const core = /* @__PURE__ */ new Set([rag.torso, rag.pelvis, rag.parts.get("head").body]);
  ...
  for (let k = 0; k < dirs.length; k++) {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      for (const b of rag.bodies) {
        const w = b.angularVelocity.length();
        if (core.has(b)) {
          coreMaxW = Math.max(coreMaxW, w);
          if (w > 19.9) coreCapHits++;
  ...
  console.log();
  check(
    "\uAE09\uD68C\uC804/\uC810\uD504\uB97C \uC11E\uC5B4\uB3C4 \uCF54\uC5B4(\uBAB8\uD1B5/\uACE8\uBC18/\uBA38\uB9AC)\uB294 \uD074\uB7A8\uD504\uC5D0 \uB2FF\uC9C0 \uC54A\uB294\uB2E4",
    coreCapHits === 0,
    `coreMaxW=${coreMaxW.toFixed(1)} coreCapHits=${coreCapHits}`
  );
  check(
    "\uAE09\uD68C\uC804/\uC810\uD504 \uC911 \uAD00\uC808\uC774 \uD06C\uAC8C \uBC8C\uC5B4\uC9C0\uC9C0 \uC54A\uB294\uB2E4 (< 18cm, \uCC29\uC9C0 \uCDA9\uACA9 \uD3EC\uD568)",
    maxGap < 0.18,
    `maxGap=${maxGap.toFixed(3)}m`
  );
  ...
  check(
    "\uC77C\uC5B4\uB09C \uB4A4 \uACE8\uBC18 \uB192\uC774\uAC00 \uC815\uC0C1",
    Math.abs(rag.pelvis.position.y - P.rideHeight) < 0.2,
