...
function applyCarryForce(physics, body, holders) {
  ...
  for (const h of holders) {
    ...
    const fx2 = fwd.x / fLen, fz2 = fwd.z / fLen;
    tx += h.rag.torso.position.x + fx2 * (P.carryDist + halfDepthAlong(body, fx2, fz2));
    ty += h.rag.torso.position.y + P.carryHeight;
    tz += h.rag.torso.position.z + fz2 * (P.carryDist + halfDepthAlong(body, fx2, fz2));
  }
  ...
  let fy = 0;
  if (liftable) {
    fy = weight + (ty - body.position.y) * P.carryKp * body.mass - body.velocity.y * P.carryKd * body.mass;
  ...
  if (liftable) {
    ...
    if (il > 0.01) {
      const tvx = ix / il * P.maxSpeed * P.pushSpeedFactor;
      const tvz = iz / il * P.maxSpeed * P.pushSpeedFactor;
  ...
  if (hm > pushBudget) {
    const s = pushBudget / hm;
    fx *= s;
    fz *= s;
  }
  if (liftable) {
    body.applyForce(new CANNON2.Vec3(fx, fy, fz));
  } else {
    const rp = new CANNON2.Vec3(0, -halfHeight(body) * P.pushLowRatio, 0);
    body.applyForce(new CANNON2.Vec3(fx, 0, fz), rp);
  }
  const per = 1 / holders.length;
  for (const h of holders) {
    h.rag.pelvis.applyForce(new CANNON2.Vec3(-fx * per, -fy * per, -fz * per));
    if (!liftable) {
      const excess = h.rag.pelvis.position.y - P.rideHeight;
      if (excess > 0) {
        const down = Math.min(P.antiHangMax, excess * P.antiHangK) + Math.max(0, h.rag.pelvis.velocity.y) * P.antiHangDamp;
        h.rag.pelvis.applyForce(new CANNON2.Vec3(0, -down, 0));
      }
    }
  }
  const up = new CANNON2.Vec3(0, 1, 0);
...
function halfDepthAlong(body, dx, dz) {
  ...
  if (!(shape instanceof CANNON2.Box)) return 0;
  const h = shape.halfExtents;
  let sum = 0;
  const axis = new CANNON2.Vec3();
  for (const [ax, ext] of [
    [new CANNON2.Vec3(1, 0, 0), h.x],
    [new CANNON2.Vec3(0, 1, 0), h.y],
    [new CANNON2.Vec3(0, 0, 1), h.z]
  ]) {
...
function holdForceFor(body, gMag) {
...
var near = (a, b, eps = 1e-6) => Math.abs(a - b) < eps;
console.log("\n--- BUG 1: A/D \uC88C\uC6B0 \uB9E4\uD551 ---");
{
  ...
  check(
    "yaw=90\xB0 \uC774\uBA74 forward = +X",
    near(b1.fx, 1) && Math.abs(b1.fz) < 1e-9,
    `f=(${b1.fx.toFixed(3)},${b1.fz.toFixed(3)})`
  );
  check(
    "yaw=90\xB0 \uC774\uBA74 right = +Z",
    near(b1.rz, 1, 1e-9) && Math.abs(b1.rx) < 1e-9,
  ...
  for (const yaw of [0, 0.7, 1.9, -2.4, Math.PI]) {
    ...
    check(
      `yaw=${yaw.toFixed(1)} : right = forward \xD7 up`,
...
console.log("\n--- BUG 2: \uB9C8\uC6B0\uC2A4 pitch \uBC29\uD5A5 ---");
{
  const applyPitch = (pitch, movementY) => pitch + movementY * 22e-4;
  const start = 0.4;
  const up = applyPitch(start, -100);
  const down = applyPitch(start, 100);
  check("\uB9C8\uC6B0\uC2A4\uB97C \uC704\uB85C \uC62C\uB9AC\uBA74 pitch\uAC00 \uAC10\uC18C (= \uD558\uB298\uC744 \uBCF8\uB2E4)", up < start, `${start} -> ${up}`);
  check("\uB9C8\uC6B0\uC2A4\uB97C \uC544\uB798\uB85C \uB0B4\uB9AC\uBA74 pitch\uAC00 \uC99D\uAC00 (= \uBC14\uB2E5\uC744 \uBCF8\uB2E4)", down > start, `${start} -> ${down}`);
  const camY = (pitch) => Math.sin(pitch) * 6.2 + 1.8;
  check("pitch\uAC00 \uC791\uC744\uC218\uB85D \uCE74\uBA54\uB77C\uAC00 \uB0AE\uB2E4 (= \uC62C\uB824\uB2E4\uBCF8\uB2E4)", camY(up) < camY(down), `${camY(up)} vs ${camY(down)}`);
}
function buildWorld() {
  const physics = new CANNON3.World({ gravity: new CANNON3.Vec3(0, -18, 0) });
  physics.broadphase = new CANNON3.SAPBroadphase(physics);
  physics.allowSleep = false;
  physics.solver.iterations = 22;
  physics.solver.tolerance = 5e-4;
  const groundMat = new CANNON3.Material("ground");
  const playerMat = new CANNON3.Material("player");
  const propMat = new CANNON3.Material("prop");
  physics.addContactMaterial(new CANNON3.ContactMaterial(groundMat, playerMat, { friction: 0.55, restitution: 0 }));
  physics.addContactMaterial(new CANNON3.ContactMaterial(groundMat, propMat, { friction: 0.2, restitution: 0.05 }));
  physics.addContactMaterial(new CANNON3.ContactMaterial(playerMat, propMat, { friction: 0.3, restitution: 0.05 }));
  const ground = new CANNON3.Body({ type: CANNON3.Body.STATIC, shape: new CANNON3.Plane(), material: groundMat });
  ground.quaternion.setFromEuler(-Math.PI / 2, 0, 0);
  physics.addBody(ground);
  return { physics, scene: new THREE2.Scene(), groundMat, playerMat, propMat };
}
function addProp(physics, propMat, size, pos, mass) {
  const body = new CANNON3.Body({
    mass,
    shape: new CANNON3.Box(new CANNON3.Vec3(size[0] / 2, size[1] / 2, size[2] / 2)),
    position: new CANNON3.Vec3(pos[0], pos[1], pos[2]),
    material: propMat
  });
...
function spawn(physics, scene, playerMat, id, x, z) {
  const myGroup = groupFor(id);
  return createRagdoll(
    physics,
    scene,
    new CANNON3.Vec3(x, P.rideHeight + 0.15, z),
    playerMat,
...
function step(physics, rags, n, input = NONE) {
  for (let i = 0; i < n; i++) {
...
console.log("\n--- BUG 3: \uCE90\uB9AD\uD130 <-> \uC18C\uD488 \uCDA9\uB3CC ---");
{
  ...
  check(
    "\uC18C\uD488 \uBC14\uB514 \uADF8\uB8F9 = 1 (GROUP_WORLD)",
    box.collisionFilterGroup === GROUP_WORLD,
  ...
  check(
    "\uC0C1\uC790\uB97C \uD1B5\uACFC\uD574 \uC9C0\uB098\uAC00\uC9C0 \uC54A\uB294\uB2E4 (\uCE90\uB9AD\uD130\uAC00 \uC0C1\uC790\uBCF4\uB2E4 \uB4A4)",
    rag.pelvis.position.z < box.position.z,
    `pelvisZ=${rag.pelvis.position.z.toFixed(2)} boxZ=${box.position.z.toFixed(2)}`
  );
}
{
  const { physics, scene, playerMat, propMat } = buildWorld();
  const fridge = addProp(physics, propMat, [1.2, 2.2, 1], [0, 1.1, 2], 20);
  const rag = spawn(physics, scene, playerMat, 1, 0, 0);
  step(physics, [rag], 120);
  step(physics, [rag], 300, { moveX: 0, moveZ: 1, jump: false });
  const gap = fridge.position.z - rag.pelvis.position.z;
  check(
    "\uB0C9\uC7A5\uACE0\uB97C \uBAB8\uC73C\uB85C \uD1B5\uACFC\uD558\uC9C0 \uBABB\uD55C\uB2E4 (\uACE8\uBC18\uC774 \uB0C9\uC7A5\uACE0 \uC55E\uBA74 \uBC14\uAE65)",
    gap > 0.45,
    `gap=${gap.toFixed(3)}`
  );
  check(
    "\uB0C9\uC7A5\uACE0\uAC00 \uBAB8\uD1B5 \uBC15\uCE58\uAE30\uB85C\uB294 \uD06C\uAC8C \uC548 \uBC00\uB9B0\uB2E4 (<1.0m)",
    Math.abs(fridge.position.z - 2) < 1,
    `z=${fridge.position.z.toFixed(3)}`
  );
}
console.log("\n--- BUG 4: \uB0C9\uC7A5\uACE0 grab -> \uC6B4\uBC18 ---");
...
function canReach(body, hand, grabRadius) {
...
{
  const { physics, scene, playerMat, propMat } = buildWorld();
  const fridge = addProp(physics, propMat, [1.2, 2.2, 1], [0, 1.1, 2.2], 20);
  const rag = spawn(physics, scene, playerMat, 1, 0, 0);
  step(physics, [rag], 120);
...
}
{
  ...
  check(
    "holdForce\uAC00 \uB0C9\uC7A5\uACE0 \uBB34\uAC8C(360N) \uC774\uC0C1\uC73C\uB85C \uC7A1\uD78C\uB2E4",
    holdForce >= 20 * 18,
  ...
  let ramp = 0;
  const input = { moveX: 0, moveZ: 1, jump: false };
  for (let i = 0; i < 420; i++) {
    ramp += 1 / 60;
    rag.setHeld([fridge]);
  ...
  check(
    "\uB0C9\uC7A5\uACE0\uAC00 \uACF5\uC911\uC73C\uB85C \uB728\uC9C0 \uC54A\uB294\uB2E4 (\uD63C\uC790\uC11C\uB294 \uBABB \uB4E6)",
    fridge.position.y < 1.6,
...
}
{
  ...
  check(
    "2\uC778\uC774 \uC7A1\uC73C\uBA74 \uB0C9\uC7A5\uACE0\uAC00 \uB4E4\uB9B0\uB2E4 (y > 1.15)",
    fridge.position.y > 1.15,
    `y=${fridge.position.y.toFixed(3)}`
  );
}
console.log(`
